          ...
          <div className="w-full lg:w-1/2 order-2 lg:order-none ml-0 lg:ml-12"> {/* Responsive margin */}
            <form onSubmit={HandleSubmit} className="flex flex-col gap-2 p-6 bg-[#27272c] rounded-xl"> {/* Responsive gap */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {/* Responsive grid */}
                <Input name="text"  type="firstname" placeholder="Prénom" value={firstname} onChange={(e)=>setFirstname(e.target.value)} />
                <Input type="lastname" placeholder="Nom" value={lastname} onChange={(e)=>setLastname(e.target.value)} />
                <Input type="email" placeholder="Adresse Email" value={email} onChange={(e)=>setEmail(e.target.value)} />
                <Input type="phone" placeholder="Téléphone" value={phone} onChange={(e)=>setPhone(e.target.value)} />
              </div>
              <Select value={service} onValueChange={(Value:s)=> setService(Value)}>
