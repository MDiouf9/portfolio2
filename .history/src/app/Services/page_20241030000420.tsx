      ...
      <div className="m-10">
        ...
        >
          {services.map((service, index) => (
            <div key={index} id={service.id} className="flex-1 flex flex-col justify-center gap-6 group">
              <div className='w-full flex justify-between items-center'>
                <div className='text-5xl font-extrabold text-outline text-transparent group-hover:text-outline-hover transition-all duration-500'>{service.num}</div>
                
                {/* Lien de navigation avec l'icône et taille ajustée */}
                <Link href={service.href} scroll={false} className='w-[70px] h-[70px] rounded-full bg-white group-hover:bg-accent-hover transition-all duration-500 flex justify-center items-center hover:-rotate-180'>
                  <div className="flex items-center space-x-2">
                    <BsArrowDownRight className="text-2xl text-accent group-hover:text-white transition-colors duration-500" />
                  </div>
                </Link>
              </div>
